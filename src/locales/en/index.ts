const en = {
  welcome: "Welcome to RepsUp",
  common: {
    login: "Login",
    logout: "Logout",
    cancel: "Cancel",
    save: "Save",
    delete: "Delete",
    edit: "Edit",
    confirm: "Confirm",
    back: "Back",
    next: "Next",
    submit: "Submit",
    loading: "Loading...",
    error: "Error",
    success: "Success",
  },
  auth: {
    welcome: "Welcome",
    fitnessJourney: "Your fitness journey starts here",
    enterPhone: "Enter phone number",
    continue: "Continue",
    or: "Or",
    terms: "By continuing, you agree to our Terms of Service and Privacy Policy",
    login: {
      title: "Welcome Back",
      subtitle: "Login to continue your fitness journey",
      motivationalText: "The body achieves what the\nmind believes",
      phoneNumber: "Phone Number",
      phonePlaceholder: "Enter your phone number",
      continueButton: "Continue",
      socialLogin: "Or continue with",
      termsText: "By continuing, you agree to our Terms of Service and Privacy Policy",
    },
    verifyOtp: {
      title: "Verify Your Phone",
      subtitle: "Enter the code sent to {{phone}}",
      resend: "Resend Code",
      resendIn: "Resend code in",
      didntReceive: "Didn't receive the code?",
      verify: "Verify",
    },
  },
  tabs: {
    home: "Home",
    add: "Add Workout",
    history: "History",
    progress: "Progress",
    profile: "Profile",
  },
  workout: {
    title: "Workouts",
    addNew: "Add New Workout",
    edit: "Edit Workout",
    delete: "Delete Workout",
    noWorkouts: "No workouts yet",
    exercise: "Exercise",
    sets: "Sets",
    reps: "Reps",
    weight: "Weight",
    duration: "Duration",
    notes: "Notes",
  },
  profile: {
    settings: "Settings",
    language: "Language",
    theme: "Theme",
    notifications: "Notifications",
    about: "About",
    version: "Version",
    logout: "Logout",
  },
  settings: {
    changeLanguage: "Change Language",
    english: "English",
    arabic: "Arabic",
  },
  workouts: {
    title: "Workouts",
    createNew: "Create New Workout",
    exerciseCount: "{{count}} exercise",
    exerciseCount_plural: "{{count}} exercises",
    duration: "{{minutes}} min",
    restTime: "Rest: {{seconds}}s",
    setsReps: "{{sets}} sets × {{reps}} reps",
    emptyState: "No workouts yet. Create your first one!",
    filters: {
      all: "All",
      favorites: "Favorites",
      recent: "Recent",
    },
  },
  home: {
    location: "6th Cross Rd",
    locationSubtitle: "HAL 2nd Stage, Indiranagar...",
    greeting: "Hey !",
    name: "Buddy",
    heroMessage: "Workouts are more fun and motivating with a partner by your side",
    startWorkout: "Start Your First Workout",
    trackProgress: "Track your exercises, sets, and reps to monitor your progress.",
    newWorkout: "New Workout",
    recentWorkouts: "Recent Workouts",
    pushDay: "Push Day",
    today: "Today",
    minutes: "min",
  },
  errors: {
    networkError: "Network connection failed",
    authFailed: "Authentication failed",
    invalidInput: "Please check your input",
    serverError: "Something went wrong. Please try again.",
  },
};

// Create a type that preserves structure but allows any string values
type DeepStringify<T> = {
  [K in keyof T]: T[K] extends object ? DeepStringify<T[K]> : string;
};

export type TranslationKeys = DeepStringify<typeof en>;

export default en;
